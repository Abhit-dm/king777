...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        if (password !== user.password_hash) return res.status(401).json({ error: 'Invalid password' });
        ...
        res.json({
            ...
            token: token,
            user: { id: user.id, username: user.username, role: user.role, available_balance: user.available_balance }
        });
    } catch (err) {
...
// --- 2. MODULAR TRANSACTION ROUTES ---
// This handles the Generate Tokens and Reports API
app.use('/api/transactions', transactionRoutes);

// --- 3. SECURE USER ROUTES ---
app.get('/api/users/me', authenticateToken, async (req, res) => {
    try {
        const userQuery = await pool.query('SELECT id, username, role, available_balance FROM users WHERE id = $1', [req.user.id]);
...
app.post('/api/users/create', authenticateToken, async (req, res) => {
    ...
    try {
        const validRoles = { 'SUPER_ADMIN': ['ADMIN'], 'ADMIN': ['MASTER'], 'MASTER': ['AGENT'], 'AGENT': ['PLAYER'] };
...
app.get('/api/users/downline', authenticateToken, async (req, res) => {
    try {
        const downlineQuery = await pool.query('SELECT id, username, role, available_balance FROM users WHERE parent_id = $1 ORDER BY id DESC', [req.user.id]);
...
app.post('/api/users/transfer', authenticateToken, async (req, res) => {
    ...
    if (isNaN(transferAmount) || transferAmount <= 0) return res.status(400).json({ error: 'Invalid transfer amount' });
    ...
    try {
        ...
        if (targetRes.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(403).json({ error: 'Invalid target user' });
        }
...
app.listen(PORT, () => console.log(`King777 Backend running on port ${PORT}`));